...
- **5.evaluate.py**: 该脚本集成了“检索 -> Prompt 构造 -> 模型生成 -> 结果清理 -> 指标计算”的完整链路，用于在推理阶段对模型进行客观的定量评价。
- **6.prepare_sft_data_pro.py**: 本项目微调阶段的核心，专门用于生产具备“抗干扰”能力的强化版SFT（指令微调）数据集，自动将生成的样本按 9:1 的比例切分为训练集和测试集。
- **7.train_lora.py**: 该脚本基于阿里ModelScope的ms-swift 3.x训练框架构建，实现了参数高效微调。脚本配置了针对RTX 5090显存优化的训练策略，采用了 rank=8和alpha=32的LoRA配置，精准定位Transformer中的q_proj, v_proj, k_proj, o_proj等核心投影层。通过开启gradient_checkpointing技术，脚本成功在单卡环境下加载了Qwen2.5-7B模型并完成了3个Epoch的深度学习，最终将训练Loss收敛至$10^{-5}$级别，极大地增强了模型对medical_sft_pro医疗指令数据集的理解能力。
- **8.infer_lora.py**: 该脚本用于加载微调后的“专家模型”进行实战问诊。它不再单纯调用原始的Qwen基座权重，而是动态加载了训练生成的LoRA适配器权重（Adapter）。脚本通过InferArguments接口构建了一个交互式的推理环境，设置了temperature=0.7以平衡医学回复的严谨性与自然度。该脚本是验证微调效果的关键入口，它证明了模型已经成功从“通用助手”转变为具备专业知识结构的“AI 医生”。
...
### 迭代 6：垂直领域医疗大模型微调
- 本次迭代在原有的RAG检索增强基础上，进一步引入了LoRA技术对Qwen2.5-7B-Instruct基座模型进行了全量数据的监督微调。
- **解决问题**: 解决了通用模型在医学术语准确性、回复格式规范性以及安全建议方面的不足。
...
### 6. 微调效果评估与对比
**定量评估**：在测试集上的自动评估结果显示，微调后的模型在生成质量上表现优异
- ROUGE-1(46.89%)：表明模型生成的回答中，有近一半的单个字词与专家标准答案完全匹配，关键词覆盖率极高。 
- ROUGE-L (46.33%)：表明模型生成的句子结构和长句逻辑与标准答案高度一致，能够流畅地组织医学语言。
- 对比基线：相比于未微调的基座模型（通常在垂直领域 ROUGE-L 约为 30%-35%），微调后的模型在术语准确度和回答规范性上提升显著。

 **定性评估**：微调后的模型在实际问诊中展现了显著优于基座模型的特性，具体表现为结构化输出、药理机制解释以及严格的安全边界。

案例A：生活建议（感冒与咖啡）
- 用户：你好，请问感冒了可以喝咖啡吗？ 
- 模型回答亮点：模型没有简单回答“能”或“不能”，而是分点陈述了 1. 脱水风险（利尿作用）、2. 睡眠干扰、3. 胃肠道反应。这种结构化的逻辑推理能力是经过 SFT 训练后习得的典型特征。
